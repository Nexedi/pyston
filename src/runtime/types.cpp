...
static int type_set_abstractmethods(PyTypeObject* type, PyObject* value, void* context) noexcept {
    ...
    */
    ...
    if (value != NULL) {
        ...
        if (abstract < 0)
            return -1;
        // type->setattr(internStringMortal("__abstractmethods__"), value, NULL);
        //    static BoxedString* module_str = internStringImmortal("__module__");
    // type->setattr(module_str, value, NULL);
        res = PyDict_SetItemString(type->tp_dict, "__abstractmethods__", value);
    } else {
        abstract = 0;
        // type->delattr(internStringMortal("__abstractmethods__"), NULL);
        res = PyDict_DelItemString(type->tp_dict, "__abstractmethods__");
        if (res && PyErr_ExceptionMatches(PyExc_KeyError)) {
            PyErr_SetString(PyExc_AttributeError, "__abstractmethods__ FUck?");
            return -1;
        }
    }
    if (res == 0) {
...
static void typeSetAbstractMethods(Box* _type, PyObject* value, void* context) {
    ...
    static BoxedString* abstract_methods_str = internStringImmortal("__abstractmethods__");
    type->setattr(abstract_methods_str, value, NULL);
        PyType_Modified(type);
        if (abstract)
            type->tp_flags |= Py_TPFLAGS_IS_ABSTRACT;
        else
...
void setupRuntime() {
    ...
    type_cls->giveAttr("__abstractmethods__", new (pyston_getset_cls) BoxedGetsetDescriptor("__abstractmethods__", 
                                                  typeAbstractMethods, typeSetAbstractMethods, NULL));
